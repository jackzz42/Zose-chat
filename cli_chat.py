...
class TerminalChat:
    ...
    async def connect(self):
        uri = f"ws://localhost:5000/ws/{self.room_id}"
    ...
    async def handle_messages(self):
        while True:
            try:
                ...
                data = json.loads(self.crypto.decrypt_message(
                    self.crypto.current_key,
                    message
                ))
                
                if data['type'] == 'user_joined':
                    self.connected_users.add(data['username'])
                    self.display_message(None, f"{data['username']} joined the room", True)
                    self.update_users_list()
                
                elif data['type'] == 'user_left':
                    self.connected_users.remove(data['username'])
                    self.display_message(None, f"{data['username']} left the room", True)
                    self.update_users_list()
